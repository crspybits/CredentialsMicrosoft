...
// Adapted from IBM ClaimsOpenID, see https://github.com/IBM-Swift/Swift-JWT
class MicrosoftClaims: Claims {
    public init(
        iss: String,
        sub: String,
        oid: String,
...
extension MicrosoftClaims {
    // Returns the rightmost non-zero digits of the oid claim
    static func getUserIdentifier(idToken: String) -> String? {
        let jwt:JWT<MicrosoftClaims>
        do {
            jwt = try JWT<MicrosoftClaims>(jwtString: idToken, verifier: .none)
        } catch {
            return nil
        }
        
        var oid = jwt.claims.oid
        oid.removeAll { $0 == "-" }
        
        // I'm not sure, but seems like removing the leading 0's is the right thing to do.
        // I.e., assuming that Microsoft identifiers don't have leading 0's.
        while oid.first == "0" {
            oid.removeFirst()
        }
